...
export default function ChartTokenSelector(props) {
  ...
  // Initialize favorites list as an empty array to avoid undefined access before useEffect runs
  ...
  const listTokens = useMemo(() => {
    const favorites = Array.isArray(listFavoriteLocal) ? listFavoriteLocal : [];

    if (tabIndex === 1) {
      return options;
    } else {
      return options?.filter((e) => favorites.includes(e?.symbol));
    }
  ...
  const getPrice = (symbol) => {
    // Safely derive the current price for a symbol.
    // The backend price map may not be loaded yet or may not contain the symbol.
    if (!listPrice) {
      return null;
    }
    ...
    if (symbolInfo.isNative) {
      symbolInfo = getWrappedToken(chainId) || symbolInfo;
    }

    const rawPrice = listPrice[symbolInfo.address];
    if (!rawPrice) {
      return null;
    }

    // Return a JS number formatted from the on-chain USD-decimal value
    return parseFloat(formatAmount(rawPrice, USD_DECIMALS, 4));
  };
